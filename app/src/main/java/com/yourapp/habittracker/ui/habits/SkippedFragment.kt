...
import com.yourapp.habittracker.databinding.FragmentSkippedBinding
...
class SkippedFragment : Fragment() {
    ...
    private fun setupRecyclerView() {
        ...
        binding.rvSkipped.apply {
            layoutManager = LinearLayoutManager(requireContext())
            adapter = this@SkippedFragment.adapter
        }
    }
    ...
    private fun observeData() {
        viewLifecycleOwner.lifecycleScope.launch {
            ...
            viewModel.activeHabits.collectLatest { habits ->
                val skippedList = habits.filter { habit ->
                    val log = viewModel.getLogByDate(habit.id, today)
                    log != null && log.status == "skipped"
                }
                adapter.submitList(skippedList)
            }
        }
    }
    ...
    companion object {
        fun newInstance() = SkippedFragment()
    }
}